...
AFRAME.registerComponent("markerhandler", {
  ...
  getModelDistance: function (elA, elB) {
    return elA.object3D.position.distanceTo(elB.object3D.position);
  },
  presentInArray: function(arr, val) {
    for (var i of arr) {
      if (i.model_name === val) {
        return true;
      }
  ...
  tick: async function() {
    if (modelList.length > 1) {
      var isBaseModelPresent = this.presentInArray(modelList, "base");
      ...
      if (!isBaseModelPresent) {
        ...
        this.placeModel("building", models);
  ...
  getModels: function() {
    return fetch("js/buildiingList.json")
      .then(res => res.json())
      .then(data => data);
  },
